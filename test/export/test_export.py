...
def is_predispatch_test(test_name):
    return test_name.endswith(PREDISPATCH_SUFFIX)
...
graph():
    ...
    def test_nested_module_with_init_buffer(self):
        ...
        if is_predispatch_test(self._testMethodName):
            self.assertExpectedInline(
                str(ep.graph).strip(),
                """\
graph():
    %x : [num_users=2] = placeholder[target=x]
    %full : [num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([3, 3], 1), kwargs = {dtype: torch.float32, layout: torch.strided, device: cpu, pin_memory: False})
    %add : [num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%x, %full), kwargs = {})
    %mul : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%add, %x), kwargs = {})
    return (mul,)""",
            )
...
graph():
    ...
    %alias : [num_users=1] = call_function[target=torch.ops.aten.alias.default](args = (%clone,), kwargs = {})
    ...
    %add : [num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%x, %alias_2), kwargs = {})
    %mul : [num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%add, %x), kwargs = {})
    return (mul,)""",
            )
